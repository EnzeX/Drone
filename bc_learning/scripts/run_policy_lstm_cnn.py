...
from train_bc_lstm_cnn import LSTMPolicyNet

# Parameters
MODEL_PATH = os.path.expanduser("~/bc_data/airsim_bc_policy_lstm_cnn.pth")
...
latest_vel = None
latest_alt = None
hidden_state = None  # LSTM hidden state - this is the memory!


def reset_memory():
    """Reset LSTM memory - call when starting a new episode"""
    global hidden_state
...
def image_callback(msg):
    ...
    # Initialize hidden state on first call
    ...
    try:
        ...
        # Prepare tensors
        img_tensor = transform(img).unsqueeze(0).unsqueeze(0).to(device)  # [1, 1, 3, 120, 160]
        ...
        with torch.no_grad():
            ...
            action = action_output.squeeze(0).squeeze(0).cpu().numpy()  # [4]
        ...
        text3 = f"MEMORY: Active (h_norm={torch.norm(hidden_state[0]).item():.2f})"
        ...
        cv2.imshow("BC Policy with LSTM Memory", debug_img)
        key = cv2.waitKey(1)
        
        # Press 'r' to reset memory manually
        if key == ord('r'):
            reset_memory()
...
if __name__ == "__main__":
    rospy.init_node("airsim_bc_lstm_policy")
    pub = rospy.Publisher(CMD_TOPIC, Twist, queue_size=1)
    
    rospy.Subscriber(IMAGE_TOPIC, Image, image_callback)
    rospy.Subscriber(ODOM_TOPIC, Odometry, odom_callback)
    
    rospy.loginfo("🚁 LSTM-based Behavior Cloning policy running...")
    rospy.loginfo("📝 Memory will persist across observations")
    rospy.loginfo("⌨️  Press 'r' in the image window to reset memory")
